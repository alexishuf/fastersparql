...
public final class StrongDedup<R> extends Dedup<R> {
    ...
    public StrongDedup(RowType<R> rt, int initialCapacity, int weakenAt) {
    ...
    public static <R> StrongDedup<R> strongUntil(RowType<R> rt, int strongCapacity) {
        return new StrongDedup<>(rt, Math.max(8, strongCapacity>>4) , strongCapacity);
    }

    public static <R> StrongDedup<R> strongForever(RowType<R> rt, int initialCapacity) {
        return new StrongDedup<>(rt, initialCapacity, MAX_VALUE);
    }

    @Override public int capacity() {
        long sum = 0;
        for (Bucket b : buckets)
            sum += b == null ? bucketCapacity : b.rows.length;
    ...
    @Override public boolean isDuplicate(R row, int source) {
        ...
        try {
            ...
            if (bucket.add(row, hash)) // do not increment size if row overwrote an weak insertion
                ++tableSize;
            return false;
        } finally {
            lock.unlock();
    ...
    /**
     * Whether {@code row} has been previously added by {@link Dedup#isDuplicate(Object, int)}
     * or {@link StrongDedup#add(Object)}
     */
    @Override public boolean contains(R row) {
        int hash = rt.hash(row);
        var bucket = buckets[hash & bucketsMask];
        return bucket != null && bucket.contains(row, hash);
    }
    ...
    private static final class Bucket {
        ...
        void weaken() {
            int deficit = Math.max(8, rows.length>>3) -  (rows.length-size);
            if (deficit > 0) {
                hashes = copyOf(hashes, hashes.length+deficit);
                rows   = copyOf(  rows,   rows.length+deficit);
            }
            weakBegin = size;
        }
        ...
        boolean contains(Object row, int hash) {
            ...
            int weakBegin = this.weakBegin, capacity = rows.length, i = 0;
            while (i < weakBegin && (hashes[i] != hash || !rt.equalsSameVars(row, rows[i])))
                ++i;
            if (i == weakBegin) {
                if (weakBegin < capacity) {
                    i = weakBegin + hash % (capacity - weakBegin);
                    return hashes[i] == hash && rt.equalsSameVars(row, rows[i]);
                }
                return false;
            }
            return true;
        ...
        boolean add(Object row, int hash) {
            bitset |= 1L << (hash & 63);
            ...
            if (weakBegin < capacity) {
                ...
                incSize = true;
                if ((i = size) == capacity) {
                    hashes = copyOf(hashes, capacity += Math.min(capacity<<1, 128));
                    rows = copyOf(rows, capacity);
                    weakBegin = capacity;
                }
            }
            hashes[i] = hash;
        ...
        <R, E extends Throwable> void forEach(ThrowingConsumer<R, E> consumer) throws E {
            for (Object r : rows) {
                if (r != null) //noinspection unchecked
                    consumer.accept((R) r);
            }
        }
    }
