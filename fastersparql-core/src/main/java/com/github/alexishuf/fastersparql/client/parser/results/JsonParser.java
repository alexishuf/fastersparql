...
public class JsonParser implements ResultsParser {
    ...
    private final Vars vars = new Vars.Mutable(10);
    ...
    static class SyntaxException extends Exception {
        @SuppressWarnings("unused") public final long pos;
    ...
    private static final long[] WS_COMMA = alphabet(",").whitespace().get();
    private static final long[] WS_COLON = alphabet(":").whitespace().get();

    enum Token {
        /* --- --- --- JSON interstitial non-terminals --- --- --- */
        VALUE {
            @Override void parse(JsonParser s) throws SyntaxException {
                Token.parseByFirst(s, WS);
            }
        },
        R_VALUE {
            @Override void parse(JsonParser s) throws SyntaxException {
                Token.parseByFirst(s, WS_COLON);
            }
        ...
        OBJ {
            ...
            @Override void parse(JsonParser s) throws SyntaxException {
                s.cursor = skip(s.input, s.cursor, s.input.length(), WS_COMMA);
                if (s.readIf('}')) {
                    s.pop();
                } else {
        ...
        NUMBER {
            private final long[] UNTIL_END_CHARS = alphabet(",]}").invert().get();
            @Override String first() { return "-0123456789"; }
            @Override void parse(JsonParser s) throws SyntaxException {
                int end = skip(s.input, s.cursor, s.input.length(), UNTIL_END_CHARS);
                if (end < s.input.length()) {
                    if (s.atField(Field.BOOLEAN)) {
                        ...
                        if (!s.varsDone) s.notifyVars(Vars.EMPTY);
        ...
        String first() { return ""; }
        private static void parseByFirst(JsonParser s, long[] skipAlphabet) throws SyntaxException {
            s.cursor = skip(s.input, s.cursor, s.input.length(), skipAlphabet);
            if (s.hasInput()) {
    ...
    private void notifyVars(Vars vars) throws SyntaxException {
        if (varsDone)
    ...
    @Nullable CharSequence readString() throws SyntaxException {
        int open = skip(input, cursor, input.length(), WS_COMMA);
    ...
    String takeNT() throws SyntaxException {
        ...
        String nt;
        if ("uri".equalsIgnoreCase(type) || "ntIRI".equalsIgnoreCase(type)) {
            nt = "<"+value+">";
