 ...
 * {@link CallbackPublisherOld#feed(Object)} method one or more times and eventually calls the
 * {@link CallbackPublisherOld#complete(Throwable)} method. The
 * {@link CallbackPublisherOld#complete(Throwable)} method is idempotent, so only the first call
 * will have an effect.
 *
 * {@link CallbackPublisherOld#feed(Object)} and {@link CallbackPublisherOld#complete(Throwable)} can
 * be called even before a {@link CallbackPublisherOld#subscribe(Subscriber)}. Early notifications
 ...
 * {@link CallbackPublisherOld#onRequest(long)} or {@link CallbackPublisherOld#onBackpressure} method.
 * The former is called for every positive {@link Subscription#request(long)} and the latter
 * when the original producer {@link CallbackPublisherOld#feed(Object)}s an item beyond the currently
 * requested amount (which is always the case if there is no subscriber yet).
 *
 * @param <T>
 */
@Deprecated
public abstract class CallbackPublisherOld<T> implements Publisher<T> {
    private static final Logger log = LoggerFactory.getLogger(CallbackPublisherOld.class);
    private static final AtomicInteger nextAnonId = new AtomicInteger(1);

    private final String name;
    private final ReactiveEventQueue<T> queue;

    public CallbackPublisherOld(@Nullable String name) {
        ...
        queue = new ReactiveEventQueue<T>(this.name) {
            ...
            @Override protected void onRequest(long n) {
                CallbackPublisherOld.this.onRequest(n);
            }
     ...
     * cancelled or {@link CallbackPublisherOld#complete(Throwable)} has already been called,
     ...
     * {@link CallbackPublisherOld#feed(Object)}.
     *
     * An implementation should instruct the original callback-based publisher to pause production.
     * Production should resume quickly once {@link CallbackPublisherOld#onRequest(long)} is called.
     */
