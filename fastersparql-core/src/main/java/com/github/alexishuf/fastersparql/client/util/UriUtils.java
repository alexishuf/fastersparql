...
public class UriUtils {
     ...
     * </pre>
     */
    private static final long[] FORBIDDEN_QUERYPARAM = Skip.alphabet(
            ";/?:@&=+$,", // reserved
            "<>#%\""        , // delims
            "{}|\\^[]`"       // unwise
    ).control().whitespace().get();
    private static final long[] SAFE_QUERYPARAM = Skip.alphabet(FORBIDDEN_QUERYPARAM).invert().get();
        ...
        for (int i = begin; i < end; i++) {
            ...
            switch (state) {
                case 0:
                    ...
                    else if (Skip.contains(FORBIDDEN_QUERYPARAM, c)) return true;
                    break;
                case 1:
    ...
    private static StringBuilder doEscapeQueryParam(CharSequence in, int begin, int end,
                                                    StringBuilder out) {
        out.ensureCapacity(out.length() + 2*(end - begin));
        for (int i, consumed = begin; consumed < end; consumed = i+1) {
            i = skip(in, consumed, end, SAFE_QUERYPARAM);
            if (i < end) {
