...
import com.github.alexishuf.fastersparql.client.model.batch.adapters.CallbackBIt;
...
class MergeBItTest extends AbstractMergeBItTest {
    @Override protected void run(Scenario scenario) {
        ...
        try (var bit = new MergeBIt<>(s.operands(), Integer.class)) {
    ...
    void testCloseBlockedDrainers(int n) throws Exception {
        for (int repetition = 0; repetition < 16; repetition++) {
            var sources = range(0, n).mapToObj(i -> new CallbackBIt<>(Integer.class)).toList();
            var active = new AtomicBoolean(true);
            Thread feeder = null;
            try (var it = new MergeBIt<>(sources, Integer.class)) {
                it.minBatch(2).minWait(10, MILLISECONDS);
                if (n <= 3) {
                    it.maxWait(20, MILLISECONDS);
                    sources.get(0).feed(1);
                } else {
                    feeder = ofVirtual().start(() -> range(0, n - 3).forEach(i -> {
                        if (active.get())
    ...
    void testMinWait(BatchGetter getter) {
        ...
        try (var s1 = new CallbackBIt<>(Integer.class);
             var s2 = new CallbackBIt<>(Integer.class);
             var s3 = new CallbackBIt<>(Integer.class);
             var it = new MergeBIt<>(List.of(s1, s2, s3), Integer.class)) {
    ...
    void testMaxWait(BatchGetter getter) {
        ...
        try (var s1 = new CallbackBIt<>(Integer.class);
             var s2 = new CallbackBIt<>(Integer.class);
             var it = new MergeBIt<>(List.of(s1, s2), Integer.class)) {
