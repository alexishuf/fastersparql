...
class BatchTest {
    ...
    @ParameterizedTest @ValueSource(ints = {0, 1, 2, 3, 4, 9, 10, 11})
    void testRemove(int size) {
        IntStream.range(0, size).forEach(start -> IntStream.range(0, size-start+2).forEach(length -> {
            String msg = "[start=" + start + ", length=" + length+']';
            var origin = range(1, size + 1).boxed().toList();
            var expected = new ArrayList<>(origin);
            expected.removeIf(i -> i >= 1+start && i < 1+start+length);

            Batch<Integer> b = new Batch<>(Integer.class, 10);
            origin.forEach(b::add);
            b.remove(start, length);
            List<Integer> observed = new ArrayList<>();
            assertEquals(expected.size(), b.drainTo(observed), "size mismatch "+msg);
            assertEquals(expected, observed, "items mismatch "+msg);
        }));
    }
}