...
import static com.github.alexishuf.fastersparql.store.index.LocalityCompositeDict.SH_ID_BIT;
import static com.github.alexishuf.fastersparql.store.index.LocalityCompositeDict.SH_ID_SUFF;
...
import static com.github.alexishuf.fastersparql.store.index.Splitter.SharedSide.fromConcatChar;
            ...
            for (long id = 1; id <= nStrings; ++id) {
                ...
                if (embedSharedId) {
                    ...
                    if (fromConcatChar(in.get(JAVA_BYTE, off+4)) == SUFFIX)
                        shId |= SH_ID_SUFF; // encode suffix operator !
                    off += 5; // do not copy base64 prefix
                    len -= 5;
                    outOff |= (long)shId << SH_ID_BIT;
                }
