...
import static com.github.alexishuf.fastersparql.operators.reorder.AvoidCartesianJoinReorderStrategy.isProduct;
...
class AvoidCartesianJoinReorderStrategyTest {
    private static final SparqlClient<String[], String, byte[]> client
            = new DummySparqlClient<>(ArrayRow.STRING, byte[].class);
    ...
    void test(List<String> operands, boolean useBind, List<Integer> expected) {
        ...
        var plans = operands.stream().map(sparql -> FSOps.query(client, sparql)).toList();
        var plansCopy = new ArrayList<>(plans);
        var expectedOrder = expected.stream().map(plans::get).collect(toList());
        var reorder = s.reorder(plans, useBind);
        assertEquals(expectedOrder, reorder);
        assertEquals(plansCopy, plans, "plans list was mutated");
    }

    @SuppressWarnings("unused") static Stream<Arguments> testIsProduct() {
        return Stream.of(
                /*  1 */arguments(Vars.of("x"),      "SELECT ?x ?y { ?x ?p ?y}", false, false),
                /*  2 */arguments(Vars.of("x"),      "SELECT ?y { ?x ?p ?y}",    false,  true),
                /*  3 */arguments(Vars.of("x"),      "SELECT ?y { ?x ?p ?y}",    true,  false),
                /*  4 */arguments(Vars.EMPTY,        "SELECT ?y { ?x ?p ?y}",    false,  true),
                /*  5 */arguments(Vars.EMPTY,        "SELECT ?y { ?x ?p ?y}",    true,   true),
                /*  6 */arguments(Vars.of("x", "y"), "SELECT * {?s ?p ?o}",      false,  true),
                /*  7 */arguments(Vars.of("x", "y"), "SELECT * {?s ?p ?o}",      true,   true),
                /*  8 */arguments(Vars.of("x", "o"), "SELECT ?s {?s ?p ?o}",     false,  true),
                /*  9 */arguments(Vars.of("x", "o"), "SELECT ?s {?s ?p ?o}",     true,  false),
                /* 10 */arguments(Vars.of("o", "x"), "SELECT ?s {?s ?p ?o}",     true,  false)
        );
    }

    @ParameterizedTest @MethodSource
    public void testIsProduct(Vars acc, String rightSparql, boolean useBind,
                              boolean expected) {
        assertEquals(expected, isProduct(acc, FSOps.query(client, rightSparql), useBind));
    }
}